...
type FormData = z.infer<typeof createBandSchema>;

interface CreateSalaryBandModalProps {
  open: boolean;
  onClose: () => void;
}

export function CreateSalaryBandModal({ open, onClose }: CreateSalaryBandModalProps) {
  const { toast } = useToast();
  const utils = trpc.useUtils();

  const createBand = trpc.salaryBands.create.useMutation({
    onSuccess: () => {
      utils.salaryBands.list.invalidate();
      toast({
        title: 'Bande créée',
        description: 'La bande salariale a été créée avec succès',
      });
      onClose();
    },
    onError: (error) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle className="text-2xl">Créer une bande salariale</DialogTitle>
          <DialogDescription>
            Définissez une nouvelle fourchette salariale
          </DialogDescription>
        </DialogHeader>
