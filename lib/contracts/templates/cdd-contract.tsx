/**
 * CDD Contract (Contrat à Durée Déterminée) PDF Template
 *
 * Fixed-term employment contract template for French-speaking West Africa
 * Compliant with labor laws of: Côte d'Ivoire, Senegal, Burkina Faso, Mali
 *
 * Key differences from CDI:
 * - Has definite end date
 * - Requires specific termination reason (accroissement temporaire, remplacement, travaux saisonniers, etc.)
 * - Maximum 2 renewals
 * - Maximum 2 years total duration
 * - No notice period (contract ends on specified date)
 *
 * Required legal clauses:
 * - Employee and employer identification
 * - Position and job description
 * - Contract duration and end date
 * - Reason for fixed-term contract (MANDATORY)
...
interface CDDContractData {
  ...
  // Contract duration (CDD-specific)
  startDate: string;
  endDate: string;
  cddReason: string; // Reason for fixed-term contract
  renewalCount?: number; // Number of times already renewed (0, 1, or 2)
...
export function CDDContractPDF({ data }: { data: CDDContractData }) {
  ...
  const getCDDReasonLabel = (reason: string) => {
    const labels: Record<string, string> = {
      temporary_increase: 'Accroissement temporaire d\'activité',
      replacement: 'Remplacement d\'un salarié absent',
      seasonal_work: 'Travaux saisonniers',
      specific_project: 'Réalisation d\'un projet spécifique',
      temporary_mission: 'Mission temporaire',
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.title}>À DURÉE DÉTERMINÉE (CDD)</Text>
          <Text style={styles.subtitle}>N° {data.contractNumber}</Text>
        </View>

        {/* Legal Notice */}
        <View style={styles.alert}>
          <Text style={styles.alertTitle}>⚠️ CONTRAT À DURÉE DÉTERMINÉE</Text>
          <Text style={styles.alertText}>
            Ce contrat est conclu pour une durée déterminée de {calculateDuration()}.{' '}
            Il prend fin automatiquement le {formatDate(data.endDate)} sans préavis.{' '}
            Maximum 2 renouvellements possibles, durée totale maximale de 2 ans.
          </Text>
        </View>
        ...
        {/* Article 2: Duration & Reason (CDD-specific) */}
        <View style={styles.section}>
          <Text style={styles.articleTitle}>ARTICLE 2 - DURÉE ET MOTIF DU CONTRAT</Text>
          <Text style={styles.paragraph}>
            Le présent contrat est conclu pour une <Text style={styles.bold}>durée déterminée</Text>{' '}
            de {calculateDuration()}, à compter du {formatDate(data.startDate)} jusqu'au{' '}
            {formatDate(data.endDate)}.
          </Text>
          <Text style={styles.paragraph}>
            <Text style={styles.bold}>Motif de recours au CDD :</Text> {getCDDReasonLabel(data.cddReason)}
          </Text>
          {data.renewalCount !== undefined && data.renewalCount > 0 && (
            <Text style={styles.paragraph}>
              Ce contrat constitue le <Text style={styles.bold}>
              {data.renewalCount === 1 ? 'premier renouvellement' : 'deuxième renouvellement'}
              </Text> du contrat initial.
              {data.renewalCount === 2 && (
                <Text> (ATTENTION : Dernier renouvellement possible selon la loi)</Text>
              )}
            </Text>
          )}
          <Text style={styles.paragraph}>
          ...
          <Text style={styles.paragraph}>
            <Text style={styles.bold}>Important :</Text> Le contrat prend fin automatiquement à la date{' '}
            d'échéance sans qu'il soit nécessaire de donner un préavis. Aucune indemnité de licenciement{' '}
            n'est due si le contrat va à son terme normal.
          </Text>
        </View>
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.paragraph}>
            Toutefois, le contrat peut être rompu avant son terme dans les cas suivants :
          </Text>
          <Text style={styles.paragraph}>
          ...
          <Text style={styles.paragraph}>
            En cas de rupture anticipée du fait de l'employeur (hors faute grave), l'employé(e){' '}
            aura droit à des dommages et intérêts correspondant aux salaires qu'il/elle aurait{' '}
            perçus jusqu'à la fin du contrat.
          </Text>
        </View>

        {/* Article 7: Renewal */}
        <View style={styles.section}>
          <Text style={styles.articleTitle}>ARTICLE 7 - RENOUVELLEMENT</Text>
          <Text style={styles.paragraph}>
            Le présent contrat peut être renouvelé <Text style={styles.bold}>deux fois au maximum</Text>,{' '}
            à la seule initiative de l'employeur, sous réserve du respect des conditions légales.
          </Text>
          <Text style={styles.paragraph}>
            La durée totale du contrat, renouvellements compris, ne peut excéder{' '}
            <Text style={styles.bold}>2 ans</Text>.
          </Text>
          <Text style={styles.paragraph}>
            Tout renouvellement fera l'objet d'un avenant écrit avant l'échéance du contrat en cours.
          </Text>
        </View>
        ...
        <View style={styles.footer}>
          <Text>
            Contrat de travail CDD - {data.companyName} - N° {data.contractNumber}
          </Text>
          <Text>
